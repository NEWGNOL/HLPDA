	...
	<view class="container">
		...
		<view class="proreportview" v-show="TabSelectedIndex == 2" @touchstart='TouchStart' @touchend='TouchEnd'>
			...
			<scroll-view class="detailscrollview" scroll-y="true">
				...
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FSOBillNo != null) ? this.ProreportInfoItem.FSOBillNo : '空'}}</text>
				<text class="detailtitle">批号：</text>
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FGMPBatchNo != null) ? this.ProreportInfoItem.FGMPBatchNo : '空'}}</text>
				<text class="detailtitle">计划生产数量：</text>
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FAuxQty != null) ? this.ProreportInfoItem.FAuxQty : '空'}}</text>
				<text class="detailtitle">计划开工日期：</text>
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FPlanCommitDate != null) ? this.ProreportInfoItem.FPlanCommitDate : '空'}}</text>
				<text class="detailtitle">实际完工日期：</text>
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FFinishDate != null) ? this.ProreportInfoItem.FFinishDate : '空'}}</text>
				<text class="detailtitle">生产预测单号：</text>
				<text
					class="detaildata">{{(this.ProreportInfoItem != null && this.ProreportInfoItem.FPPOrderBillNo != null) ? this.ProreportInfoItem.FPPOrderBillNo : '空'}}</text>
		...
		<OutStorageKeyboard class="keyboard" @confirm="CloseQtyPopupWindow1" @exit="CloseQtyPopupWindowDirect1"
			v-show="IsOpenDigitKeyboard1"></OutStorageKeyboard>
			
		<OutStorageKeyboard class="keyboard" @confirm="CloseQtyPopupWindow2" @exit="CloseQtyPopupWindowDirect2"
			v-show="IsOpenDigitKeyboard2"></OutStorageKeyboard>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				IsOpenDigitKeyboard1: false,
				IsOpenDigitKeyboard2: false,
				UserDepartment: '',
				Main: '',
				Receiver: '',
		...
		methods: {
			...
			//打开数量弹窗
			OpenQtyPopupWindow2: function() {
				//打开键盘界面
				this.IsOpenDigitKeyboard2 = true;				
			},
			CloseQtyPopupWindowDirect: function(e) {
				//关闭键盘界面
				this.IsOpenDigitKeyboard = false;
			},
			CloseQtyPopupWindowDirect1: function(e) {
				//关闭键盘界面
				this.IsOpenDigitKeyboard1 = false;
			},
			CloseQtyPopupWindowDirect2: function(e) {
				//关闭键盘界面
				this.IsOpenDigitKeyboard2 = false;
			},
			//关闭数量弹窗
			...
			GetGblSettingBySignalKey: function() {
				uni.request({
					...
					success: (result) => {
						...
						this.IsMotorDepartment = (this.UserDepartment.indexOf(FValue) != -1 ? true :
							false);
					},
					fail: () => {
						Config.PopAudioContext(false);
